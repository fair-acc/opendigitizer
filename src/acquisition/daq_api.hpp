...
inline void storeFlowgraphToMessage(const flowgraph::Flowgraph& outFlowgraph, gr::Message& message) {
    message.data = gr::property_map{
        {std::pmr::string("serialisedFlowgraph"), outFlowgraph.serialisedFlowgraph}, //
        {std::pmr::string("serialisedUiLayout"), outFlowgraph.serialisedUiLayout}    //
    };
}
...
inline std::expected<flowgraph::Flowgraph, std::string> getFlowgraphFromMessage(const gr::Message& message) {
    ...
    const auto source = it->second.value_or(std::string_view{});
    ...
    if (it != dataMap.end()) {
        if (const auto uiLayout = it->second.value_or(std::string_view{}); uiLayout.data() != nullptr) {
            serialisedUiLayout = uiLayout;
        }
    }

    return flowgraph::Flowgraph{.serialisedFlowgraph = std::string(source), .serialisedUiLayout = serialisedUiLayout};
}
