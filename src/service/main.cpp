...
int main() {
    ...
    client.subscribe(URI("mds://127.0.0.1:12345/DeviceName/Acquisition?channelNameFilter=sine"), [&receivedA](const opencmw::mdp::Message &update) {
        fmt::print("Client('sine') received message from service '{}' for endpoint '{}'\n{}\n", update.serviceName.str(), update.endpoint.str(), update.data.asString());
        receivedA++;
    });
    client.subscribe(URI("mds://127.0.0.1:12345/DeviceName/Acquisition?channelNameFilter=sine,saw"), [&receivedAB](const opencmw::mdp::Message &update) {
        fmt::print("Client('sine,saw') received message from service '{}' for endpoint '{}'\n{}\n", update.serviceName.str(), update.endpoint.str(), update.data.asString());
        receivedAB++;
    });

    while (receivedA < 1  && receivedAB < 2) {
        std::this_thread::sleep_for(200ms);
    }

    client.unsubscribe(URI("mds://127.0.0.1:12345/DeviceName/Acquisition?channelNameFilter=sine"));
    client.unsubscribe(URI("mds://127.0.0.1:12345/DeviceName/Acquisition?channelNameFilter=sine,saw"));
