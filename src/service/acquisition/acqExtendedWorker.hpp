...
public:
    ...
    void handleGetRequest(const si::time<nanosecond, ::int64_t> updateStamp, const AcquisitionFilter &ctx, AcquisitionExtended_float &out) {
        ...
        out.signalExtents        = std::vector<std::int32_t>{ static_cast<int>(out.signalNames.size()), N_SAMPLES_T, N_SAMPLES_F };
        out.signalValues         = std::vector<float>(out.signalNames.size() * N_SAMPLES_T * N_SAMPLES_F);
        out.signalErrors         = std::vector<float>(out.signalNames.size() * N_SAMPLES_T * N_SAMPLES_F);
        ...
        std::size_t n            = 0; // signal index
        auto        signalValues = out.signalValuesMDspan<3>();
        auto        signalErrors = out.signalErrorsMDspan<3>();
        for (auto &name : out.signalNames) {
            if (name == "saw") {
                for (std::size_t t = 0; t < N_SAMPLES_T; t++) {
                    for (std::size_t f = 0; f < N_SAMPLES_F; f++) {
                        signalValues(n, t, f) = float(t % 4) * float(f % 3);
                        signalErrors(n, t, f) = 0.1f;
                    }
                }
                out.signalUnits.emplace_back("particles");
                out.signalRanges.emplace_back(-0.1f);
                out.signalRanges.emplace_back(6.1f);
                out.signalStatus.emplace_back("errors=[],temperature=20.4");
            } else if (name == "chess") {
                for (std::size_t t = 0; t < N_SAMPLES_T; t++) {
                    for (std::size_t f = 0; f < N_SAMPLES_F; f++) {
                        signalValues(n, t, f) = ((t << 2) % 2 xor (f << 1) % 2) ? 0.2f : 0.8f;
                        signalErrors(n, t, f) = 0.01f;
                    }
                }
