...
class AcquisitionWorker : public Worker<serviceName, AcquisitionFilter, Empty, Acquisition, Meta...> {
std::jthread              _notifyThread;
std::chrono::milliseconds _rate;

public:
using super_t = Worker<serviceName, AcquisitionFilter, Empty, Acquisition, Meta...>;
...
explicit AcquisitionWorker(const BrokerType &broker, std::chrono::milliseconds rate)
    ...
    super_t::setCallback([this](RequestContext &rawCtx, const AcquisitionFilter&requestContext, const Empty &, AcquisitionFilter& /*replyContext*/, Acquisition &out) {
...
void handleGetRequest(const si::time<nanosecond, ::int64_t> updateStamp, const AcquisitionFilter &ctx, Acquisition &out) {
    ...
    out.signalValues.dimensions() = {static_cast<unsigned int>(out.signalNames.size()), N_SAMPLES};
    out.signalValues.elements().resize(out.signalNames.size() * N_SAMPLES);
    out.signalErrors.dimensions() = {static_cast<unsigned int>(out.signalNames.size()), N_SAMPLES};
    out.signalErrors.elements().resize(out.signalNames.size() * N_SAMPLES);
    out.signalTimeBase.resize(N_SAMPLES);
    for (std::size_t i = 0; i < N_SAMPLES; i++) {
        // out.signalTimeBase[i] = static_cast<float>(i) * _rate.count() * 1e-3f / N_SAMPLES; // time in float seconds
        out.signalTimeBase[i] = i * _rate.count() / N_SAMPLES; // time in integer nanoseconds
    }
    ...
    for (auto &name : out.signalNames) {
        if (name == "sine") {
            ...
            for (std::size_t i = 0; i < N_SAMPLES; i++) {
                ...
                out.signalValues.elements()[n * N_SAMPLES + i] = static_cast<float>(amplitude * std::sin(static_cast<double>(t) / n_period * std::numbers::pi));
                out.signalErrors.elements()[n * N_SAMPLES + i] = 0.0f;
            }
            out.signalUnits.push_back("V");
            out.signalRangeMin.push_back(-1);
            out.signalRangeMax.push_back(1);
        } else if (name == "saw") {
            const double amplitude = 9.0;
            const int    n_period  = 13;
            for (std::size_t i = 0; i < N_SAMPLES; i++) {
                const float t       = (static_cast<float>((updateStamp.number() * N_SAMPLES / _rate.count() / 1000000 + i) % n_period)) * _rate.count() * 1e-3f / N_SAMPLES;
                out.signalValues.elements()[n * N_SAMPLES + i] = static_cast<float>(amplitude * static_cast<double>(t) / n_period);
                out.signalErrors.elements()[n * N_SAMPLES + i] = 0.01f;
            }
            out.signalUnits.push_back("A");
            out.signalRangeMin.push_back(0);
            out.signalRangeMax.push_back(9);
        }
    }
}
};
