...
struct XYChart : gr::Block<XYChart, gr::BlockingIO<false>, gr::Drawable<gr::UICategory::ChartPane, "ImGui">>, Chart {
    ...
    GR_MAKE_REFLECTABLE(XYChart, chart_name, chart_title, data_sinks, x_axis_mode, show_legend, show_tags, show_grid, anti_aliasing, max_history_count, history_opacity_decay, history_vertical_offset);
    ...
    // Note: _signalSinks is inherited from Chart base class
    // Axis config is read from uiConstraints()["axes"] in draw()
    ...
    // --- Signal sink management (override to request capacity) ---
     ...
     */
    void buildAxisCategories() { buildAxisCategoriesWithFallback(); }

    /**
     * @brief Setup all ImPlot axes.
     * Call this after BeginPlot() and before SetupFinish().
     */
    void setupAllAxes() { setupAxes(); }
    ...
    gr::work::Status draw(const gr::property_map& config = {}) {
        ...
        // Determine plot flags: show legend if show_legend property is true OR if in layout mode
        ImPlotFlags plotFlags = ImPlotFlags_NoTitle;
        if (!show_legend && !layoutMode) {
            plotFlags |= ImPlotFlags_NoLegend;
        }

        if (_signalSinks.empty()) {
            drawEmptyPlot(plotFlags);
...
private:
    ...
    /**
     * @brief Get axis config from uiConstraints()["axes"] for the given axis type and index.
     * @param isX true for X-axis, false for Y-axis
     * @param index axis index (0, 1, 2 for multiple axes)
     * @return parsed DashboardAxisConfig or nullopt if not found
     */
    [[nodiscard]] std::optional<DashboardAxisConfig> getDashboardAxisConfig(bool isX, std::size_t index) const {
        using AxisKind = DashboardAxisConfig::AxisKind;
        const AxisKind targetKind = isX ? AxisKind::X : AxisKind::Y;

        // Access ui_constraints member (Annotated property in gr::Block)
        const auto& constraints = this->ui_constraints.value;
        ...
        for (const auto& axisPmt : *axesVec) {
            ...
            if (count == index) {
                // Parse and return this axis config
                DashboardAxisConfig cfg;
                cfg.axis = parsedKind;
    ...
    void drawEmptyPlot(ImPlotFlags plotFlags = ImPlotFlags_NoTitle) {
        if (DigitizerUi::TouchHandler<>::BeginZoomablePlot(chart_name.value, ImGui::GetContentRegionAvail(), plotFlags)) {
