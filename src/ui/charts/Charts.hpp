 ...
 * @return Shared pointer to the new XYChart (also ChartInterface)
 */
 ...
 *
 * @param name Chart name (optional)
 * @return Shared pointer to the new YYChart (also ChartInterface)
 */
 ...
 * @return Pair of (shared_ptr for ownership, ChartInterface* for polymorphic access)
 *         Both are null if typeName is unknown.
 */
inline std::pair<std::shared_ptr<void>, ChartInterface*> makeChartByType(std::string_view typeName, const std::string& name = "") {
