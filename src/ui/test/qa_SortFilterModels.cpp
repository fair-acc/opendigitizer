...
const static boost::ut::suite<"SortFilterTreeModel"> sortFilterTreeModelTests = [] {
    ...
    "a branch is not created if all its children are pruned"_test = [] {
        std::map<std::string, int> invocationCounts;
        auto                       params = paramsFor(samplePaths);
        params.filters.filterObjects.emplace_back(std::make_unique<RejectSomeNamesFromSamplePathsFilter>(std::vector<std::string>{"orbit", "injection", "extraction"}, &invocationCounts));
        SortFilterTreeModel model(std::move(params));
        ...
        const bool matchesExpected = resolveSortingAndReturnTree(model) == expected;
        expect(matchesExpected);
        expect(eq(invocationCounts.size(), samplePaths.size())) << "filter ran once for each leaf";
        for (const auto& [leafName, count] : invocationCounts) {
            expect(eq(count, 1)) << "the filter should be evaluated exactly once per leaf, in this case " << leafName;
        }
    };
