...
struct ImPlotSink : ImPlotSinkBase<ImPlotSink<T>> {
    ...
    void requestCapacity(void* /*owner*/, std::size_t minSamples) {
        if (minSamples > required_size) {
            required_size = static_cast<gr::Size_t>(minSamples);
        }
    }

    void releaseCapacity(void* /*owner*/) {
