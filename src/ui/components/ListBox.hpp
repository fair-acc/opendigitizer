...
struct FilterListContext {
    ...
    [[nodiscard]] constexpr bool filter(std::string_view name) {
        if (!filterString.empty()) {
            auto it = std::ranges::search(name, filterString, [](char a, char b) { return Digitizer::utils::safe_tolower(a) == Digitizer::utils::safe_tolower(b); });
            if (it.data() == name.end()) {
                return false;
            }
        }
    ...
    std::pair<int, bool> initSelection(bool shouldScrollToSelected) {
        ...
        int  selectOffset     = 0;
        bool scrollToSelected = shouldScrollToSelected;
        if (selected) {
        ...
        return {selectOffset, scrollToSelected};
    }
    ...
    void draw(const Items& items, ItemGetter getItem, ItemDrawer drawItem, bool shouldScrollToSelected) {
        ...
        for (auto it = filteredItems.begin(); it != filteredItems.end(); ++it) {
            ...
            if (selectOffset == -1 && *(it + 1) == *selected) {
                selected     = *it;
                selectOffset = 0;
            } else if (selectOffset == 1 && *it == *selected && (it + 1) != filteredItems.end()) {
                selected     = {};
                selectOffset = 0;
            }
